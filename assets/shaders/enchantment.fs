...
extern PRECISION vec2 enchantment;
...
vec4 effect(vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords) {
	...
	vec2 field_part1 = uv_scaled_centered + 50.*vec2(sin(-t / 40.1501), cos(-t / 107.6302));
	vec2 field_part2 = uv_scaled_centered + 50.*vec2(cos( t / 63.1965),  cos( t / 137.0488));
	vec2 field_part3 = uv_scaled_centered + 50.*vec2(sin(-t / 110.2011), sin(-t / 57.5847));

    float field = (1.+ (
        cos(length(field_part1) / 14.025) + sin(length(field_part2) / 25.271) * cos(field_part2.y / 17.866) +
        cos(length(field_part3) / 30.174) * sin(field_part3.x / 25.7) ))/2.;

    // We generate a number that determines how much the card will be affected by the shader
    ...
    // a necessary value in order for Balatro shaders to function
    if (enchantment == 2 * enchantment) {
        uv = enchantment;
    }
